...
{
    private const string MUTEX_NAME = "LOQToolkit_Mutex_6efcc882-924c-4cbc-8fec-f45c25696f98";
    private const string EVENT_NAME = "LOQToolkit_Event_6efcc882-924c-4cbc-8fec-f45c25696f98";
    ...
    private static async Task InitializeBackgroundServicesAsync()
    {
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Starting background initialization...");
            ...
            var initializationTasks = new[]
            {
                ...
                StartHWiNFOIntegrationAsync(),
                StartIpcServerAsync(),
                StartBatteryDischargeRateMonitorServiceAsync(),
            };
    ...
    private static async Task StartAIControllerAsync()
    {
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Couldn't start AI controller.", ex);
        }
    }

    private static async Task StartHWiNFOIntegrationAsync()
    {
        try
        {
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Couldn't start HWiNFO integration.", ex);
        }
    }
    ...
    private static async Task StartIpcServerAsync()
    {
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Couldn't start IPC server.", ex);
        }
    }

    private static async Task StartBatteryDischargeRateMonitorServiceAsync()
    {
        try
        {
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Couldn't start battery discharge monitor.", ex);
        }
    }
