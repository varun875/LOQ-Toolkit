...
using LOQToolkit.WPF.CLI.Features;

namespace LOQToolkit.WPF.CLI;
    ...
    )
{
    ...
    private async Task Handler(CancellationToken token)
    {
        try
        {
            ...
            await using var pipe = NamedPipeServerStreamAcl.Create(LOQToolkit.CLI.Lib.Constants.PIPE_NAME,
                PipeDirection.InOut,
                1,
