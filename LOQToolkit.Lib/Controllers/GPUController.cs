...
using LOQToolkit.Lib.Resources;
...
public class GPUController
{
    ...
    private async Task RefreshStateAsync()
    {
        ...
        catch (NVIDIAApiException ex) when (IsGpuPoweredOffException(ex))
        {
    ...
    private static bool IsGpuPoweredOffException(NVIDIAApiException exception)
    {
        var message = exception.Message ?? string.Empty;
        return message.Contains("NVAPI_GPU_NOT_POWERED", StringComparison.OrdinalIgnoreCase)
            || message.Contains("NOT_POWERED", StringComparison.OrdinalIgnoreCase);
    }
}
