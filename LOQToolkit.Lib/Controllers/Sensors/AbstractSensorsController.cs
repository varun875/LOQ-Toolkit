...
namespace LOQToolkit.Lib.Controllers.Sensors;
...
public abstract class AbstractSensorsController(GPUController gpuController) : ISensorsController
{
    ...
    private async Task<GPUInfo> GetGPUInfoAsync()
    {
        ...
        try
        {
            ...
            var gpuUsage = gpu.UsageInformation?.GPU?.Percentage ?? -1;
            var videoEngineUsage = gpu.UsageInformation?.VideoEngine?.Percentage ?? -1;
            var utilization = (gpuUsage < 0 && videoEngineUsage < 0)
                ? -1
                : Math.Min(100, Math.Max(gpuUsage, videoEngineUsage));
